...
public class GerhardtSchusterTyson {
    ...
    final static int G_UP = 40;
    final static int G_DOWN = 3;
    ...
    static class Display extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < N; j++) {
                    float scale = 255*((float)v[i][j]/(float)V_MAX);
                    int num = (int) scale;
                    if (u[i][j] == 1) {
                        g.setColor(new Color(num,0,0));
                    }
                    else if (v[i][j] < V_EXCI){
                        g.setColor(Color.WHITE);
                        //g.setColor(new Color(num,num,num));
                    }
                    else {
                        g.setColor(new Color(0,0,num));
                    }
